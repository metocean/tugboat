...
var TUGBOATFormatException, fs, globalvalidation, isboolean, isnumber, isobjectofstringsornull, isrestartpolicy, isscripts, isstring, isstringarray, parse_port, preprocess, resolve, template, validation, yaml,
...
preprocess = function(config, path) {
  var chunks, content, e, env, filename, filepath, j, key, l, len, len1, ref, ref1, result, results1, value;
  ...
  if ((config.environment != null) && config.environment instanceof Array) {
    ...
    if (config.env_file != null) {
      ...
      for (l = 0, len1 = ref1.length; l < len1; l++) {
        ...
        if (!isobjectofstringsornull(content)) {
        ...
        results1.push((function() {
          ...
          for (key in content) {
            value = content[key];
            results2.push(config.environment[key] = value);
          }
          return results2;
        })());
      }
...
module.exports = function(groupname, services, path, cb) {
  ...
  for (name in services) {
    ...
    if (globals.add_hosts != null) {
      if (config.add_hosts == null) {
        config.add_hosts = [];
      }
